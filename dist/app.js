...
import { generateJWTtokens, logInValidationRules, setAuthCookies, signUpValidationRules, } from "@shared/utils/auth";
...
app.post("/api/v1/sign-up", signUpValidationRules(), validateRequest, async (req, res) => {
    ...
    try {
        ...
        const settedUser = await setUser(securedUser);
        if (!settedUser) {
...
app.post("/api/v1/log-in", logInValidationRules(), validateRequest, async (req, res) => {
    const inputUser = req.body;
    try {
        const userDBdata = await getUser(inputUser.email);
        if (!userDBdata) {
        ...
        const isPasswordEqual = await bcrypt.compare(inputUser.password, hashedPassword);
        if (!isPasswordEqual) {
            return res.status(401).json({ message: "Invalid email or password." });
        }
